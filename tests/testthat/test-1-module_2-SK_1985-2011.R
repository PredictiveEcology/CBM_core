...
test_that("Module: SK 1985-2011", {
  ...
  simInitInput <- SpaDEStestMuffleOutput(
    ...
    SpaDES.project::setupProject(
      ...
      cohortDT          = data.table::fread(file.path(paths$testdata, "SK/input", "cohortDT.csv"))[, ageSpinup := sapply(age, max, 3)],
      standDT           = data.table::fread(file.path(paths$testdata, "SK/input", "standDT.csv"))[, area := 900],
      disturbanceEvents = file.path(paths$testdata, "SK/input", "disturbanceEvents.csv") |> data.table::fread(),
