...
#### `save`
This event is where outputs are saved. In the case of our example in Saskatchewan, we save outputs to `.qs` files to our [`spadesCBMdb`](https://predictiveecology.github.io/spadesCBM/lexicon.html#spadesCBMdb) object for each simulation year. By default, 3 tables are saved, a `key` table, a `pools` table, and a `flux` table. Other tables with simulation parameters can be saved by changing the `.saveAll` parameter to `TRUE` in the module. If output paths are kept at default, hese output files will be found here: `~/GitHub/spadesCBM/outputs`. 
