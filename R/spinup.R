...
cbmExnSpinup <- function(cohortDT, spuParams, growthIncr,
                         colname_gc      = "gcids",
  ...
  # Read input tables
  reqCols <- list(
    cohortDT   = c("cohortID", "spatial_unit_id", colname_gc, colname_species, "sw_hw", colname_age),
    spuParams  = c("id", "return_interval", "min_rotations", "max_rotations", "mean_annual_temperature"),
  ...
  spuParams  <- readDataTable(spuParams,  "spuParams",  colRequired = reqCols$spuParams)
  ...
  # Isolate unique groups and join with spatial unit parameters
  cohortGroups <- unique(cohortDT[, .SD, .SDcols = c("cohortGroupID", groupCols)])
  cohortGroups <- data.table::merge.data.table(
    cohortGroups, spuParams, by.x = "spatial_unit_id", by.y = "id", all.x = TRUE)
  ...
  if (colname_age != "age"){
    if (colname_age != "age" && "age" %in% names(cohortGroups)) cohortGroups[, age := NULL]
