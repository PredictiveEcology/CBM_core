...
In our example, we specify in the [`globalSK.R`](https://github.com/PredictiveEcology/spadesCBM/blob/main/globalSK.R) script we used default saving options, which saves output tables to [`spadesCBMdb`](https://predictiveecology.github.io/spadesCBM/index.html#spadesCBMdb) every simulation year. However, a user can change the frequency at which these are saved at, or decide to save all output tables instead of the default using the `.saveInterval` and `.saveAll` parameters respectively. A user can also choose to save their `spinup` results (off by default) by switching the `.saveSpinup` parameter to `TRUE`. A user can also choose the outputs they would like to see from all objects in the [simList](https://predictiveecology.github.io/spadesCBM/lexicon.html#simlist). For example, a user my have interest in the quantity (in tonnes of carbon) of forest products and emissions existing the system. Our [simList](https://predictiveecology.github.io/spadesCBM/lexicon.html#simlist) is named `simMngedSKtestArea` (line 61 of our global script [`globalSK.R`](https://github.com/PredictiveEcology/spadesCBM/blob/main/globalSK.R)), so the object of interest would be `simMngedSKtestArea$emissionsProducts`.
