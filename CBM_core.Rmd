...
[CBM_core](https://github.com/PredictiveEcology/CBM_core.git) is the central module of [spadesCBM](https://github.com/PredictiveEcology/spadesCBM.git) implementing the [CBM-EXN](https://github.com/cat-cfs/libcbm_py/tree/main/libcbm/model/cbm_exn) model. This is where all the carbon transfers are calculated at every time step, where disturbances are applied, and stocks are tracked.

[CBM-EXN](https://github.com/cat-cfs/libcbm_py/tree/main/libcbm/model/cbm_exn) is a CBM-CFS3 equivalent model driven by external net C increments rather than merchantable volume growth curves. The [libcbmr R package](https://github.com/PredictiveEcology/libcbmr) provides an R interface to the Python implemented CBM-EXN model in [libcbm](https://github.com/cat-cfs/libcbm_py/tree/main).

CBM_core cannot run on its own and it needs parameters from [CBM_defaults](https://github.com/PredictiveEcology/CBM_defaults.git), [CBM_dataPrep_SK](https://github.com/PredictiveEcology/CBM_dataPrep_SK.git), [CBM_dataPrep](https://github.com/PredictiveEcology/CBM_dataPrep.git), and [CBM_vol2biomass](https://github.com/PredictiveEcology/CBM_vol2biomass.git) (as per the provided example for managed forests of Saskatchewan) or the equivalent information from the user. In *SpaDES-speak*, this module has six events (`Init`, `spinup`, `annual_preprocessing`, `annual_carbonDynamics`, `save`, and `plot`). Only in two of these events do carbon transactions occur: `spinup` and `annual_carbonDynamics`. The other events are tools to enable these transactions. All events are scheduled only once except for the two annual events which schedule themselves until the end of the simulation horizon.
...
| cohortDT | Data table | Table of cohort attributes | CBM_dataPrep |
...
| gcMeta | Data table | Growth curve metadata | CBM_dataPrep |
| growth_increments | Data table | growth increment matrix | CBM_vol2biomass |
| disturbanceEvents | Data table | Table with disturbance events for each simulation year | CBM_dataPrep |
...
#### `save`
This event is where outputs are saved. In the case of our example in Saskatchewan, we save outputs to `.qs` files to our [`spadesCBMdb`](https://predictiveecology.github.io/spadesCBM/lexicon.html#spadesCBMdb) object for each simulation year. By default, 3 tables are saved, a `key` table, a `pools` table, and a `flux` table. Other tables with simulation parameters can be saved by changing the `.saveAll` parameter to `TRUE` in the module. If output paths are kept at default, these output files will be found here: `~/GitHub/spadesCBM/outputs`. 
...
[CBM_core](https://github.com/PredictiveEcology/CBM_core.git) provides outputs as prescribed in the global script, the script that controls simulations. We make use of the function [SpaDES.project::setupProject](https://spades-project.predictiveecology.org/articles/i-getting-started.html#understanding-spades-projects-main-function-setupproject). While it is possible to develop a [SpaDES]() project using different methods more familiar to the user, the [SpaDES.project](https://spades-project.predictiveecology.org/) package streamlines this process, facilitating a clean, reproducible and reusable project setup for any project. 

In our example, we specify in the [`globalSK.R`](https://github.com/PredictiveEcology/spadesCBM/blob/main/globalSK.R) script we used default saving options, which saves output tables to [`spadesCBMdb`](https://predictiveecology.github.io/spadesCBM/lexicon.html#spadesCBMdb) every simulation year. However, a user can change the frequency at which these are saved at, or decide to save all output tables instead of the default using the `.saveInterval` and `.saveAll` parameters respectively. A user can also choose to save their `spinup` results (off by default) by switching the `.saveSpinup` parameter to `TRUE`. A user can also choose the outputs they would like to see from all objects in the [simList](https://predictiveecology.github.io/spadesCBM/lexicon.html#simlist). For example, a user my have interest in the quantity (in tonnes of carbon) of forest products and emissions existing the system. Our [simList](https://predictiveecology.github.io/spadesCBM/lexicon.html#simlist) is named `simMngedSKtestArea` (line 61 of our global script [`globalSK.R`](https://github.com/PredictiveEcology/spadesCBM/blob/main/globalSK.R)), so the object of interest would be `simMngedSKtestArea$emissionsProducts`.
...
| spadesCBMdb       | character  | Path to SpaDES CBM database directory containing simulation data |
